...
def create_val_dataloader_from_config(dataset_config, batch_size, sample_size, sample_rate, audio_channels=2, num_workers=4):
    ...
    if dataset_type == "audio_dir":
        ...
        return torch.utils.data.DataLoader(val_set, batch_size, shuffle=False,
                                num_workers=num_workers, persistent_workers=True, pin_memory=True, drop_last=False, collate_fn=collation_fn)
...
class SampleDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.training_type =="val":
            json_sound = "data/music_caps/json_files/musiccaps.json"
            with open(json_sound, 'r') as f:
                json_obj = json.load(f)
            self.filenames = [item["promptaudio"] for item in json_obj]
            self.prompts = [item["prompt"] for item in json_obj]
            print(f'Found validation {len(self.filenames)} files')

        ######train
        elif self.training_type =="diffusion":
            json_mtg = 'data/MTG-Jamendo/MTG-Qwen.json'
            # json_mag = "data/Magnatune/MAG-Qwen.json"
            # json_fma = "data/FMA/FMA-Qwen.json"
            json_kwai = "data/kwai/Kwai-Qwen.json"

            with open(json_mtg, 'r') as f:
                json_1 = json.load(f)
            with open(json_kwai, 'r') as f:
                json_2 = json.load(f)
    ...
    def __getitem__(self, idx):
        ...
        try:
            if self.training_type =="diffusion":
                ...
                #only train
                resampler = T.Resample(self.sr, 24000)
                prompt_audio = resampler(audio)
                if prompt_audio.shape[0] > 1:
                    prompt_audio = (prompt_audio[0] + prompt_audio[1]) / 2
                info["promptaudio"] = prompt_audio

                return (audio, info)

            elif self.training_type =="val":
                info ={}
                info["prompt"] = self.prompts[idx]
                ...
                if prompt_audio.shape[0] > 1:
                    prompt_audio = (prompt_audio[0] + prompt_audio[1]) / 2
                prompt_audio = prompt_audio[:self.sr * 10]
                info["promptaudio"] = prompt_audio

                info["path"] = audio_filename.split('/')[-1]
                info["seconds_start"] = 0
                info["seconds_total"] = 12
                return (info,)
